...
import { WhatsAppAdapter, type BaileysMessage } from "../src/adapter.js";
...
describe("WhatsAppAdapter interface compliance", () => {
  ...
  it("throws when accessing sessionDir before open", () => {
    // SecureSessionStore should throw if not opened
    // This is tested indirectly through the adapter
    const adapter = createTestAdapter();
    expect(typeof adapter.disconnect).toBe("function");
  });
});
